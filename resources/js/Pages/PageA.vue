            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200 space-y-4">
                        ...
                        <button @click="generateLuckyNumber" class="px-4 py-2 bg-blue-500 text-white font-semibold
                        ...
                        <div v-if="state.randomNumber" class="mt-6 p-4 bg-white shadow-sm rounded-lg border border-gray-200">
                            <div class="mb-4">
                        ...
                        <div>
                            <Button  v-if="!state.histories?.length"  @click="fetchHistory({'show-history' : true })" class="px-4 py-2 bg-green-500 text-white font-semibold
                            ...
                            <Button v-if="state.histories?.length"  @click="fetchHistory({})" type="button"  class="px-4 py-2 bg-green-500 text-white font-semibold
                                rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-opacity-75 transition">
                                Hide History
                            </Button>
                            {{ console.log($page.props.histories?.length) }}
                            <div v-if="state.histories?.length || histories?.length" class="mt-6">
                                <ul class="space-y-4">
                                    <li v-for="history in state.histories" :key="history.id" class="p-4 bg-white shadow-sm rounded-lg border border-gray-200">
